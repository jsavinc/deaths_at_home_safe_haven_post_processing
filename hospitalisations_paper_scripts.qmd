...
# TODO: does it matter if I remove some of the tests I didn't report first?
...
tbl_descriptives_outcomes_by_year_pod <-
  bind_rows(
    los_by_pod %>% select(cat_place_of_death, val_cohort_year, mean, sd, mean_ci_lo, mean_ci_hi) %>% mutate(outcome = "1) Days in hospital"),
    hosp_by_pod %>% select(cat_place_of_death, val_cohort_year, mean = m, sd, mean_ci_lo = ci_lo, mean_ci_hi = ci_hi) %>% mutate(outcome = "2) Number of\n hospitalisations"),
