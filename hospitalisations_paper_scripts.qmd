...
## Large table, combined numeric & categorical, home only
...
tbl_descriptives_home_only_merged_years <-
  map(
...
# TODO: delete this after gleaning what's useful
# comorb_by_pod %>%
#       filter(measure == "Elixhauser comorbidity index") %>%
#       rename(n_deaths = denominator, mean = m) %>%
#       group_by(cat_place_of_death) %>%
#       merge_years_m_sd() %>%
#       mutate(measure = "EI, Mean (SD)", value = glue::glue("{signif(mean,3)} ({signif(sd,3)})")) %>%
...
tbl_descriptives_data_numeric <- list(
  ...
  "Driving distance, Mean (SD)" = driving_by_pod %>%
    rename(mean = m),
  "Days in hospital, Mean (SD)" = los_by_pod,
...
tbl_descriptives_numeric_home_only_merged_years <-
  # TODO: sort out below script
  # tbl_descriptives_data_numeric %>%
  # group_by(cat_place_of_death) %>% 
  # merge_years_m_sd() %>%
  # mutate(
  #   measure = "Age, Mean (SD)",
